...
\begin{figure}
    ...
    \begin{minipage}[c]{.46\linewidth}
        \includegraphics[scale=0.6]{eps/Schematique_All.jpg}
        \caption{Filtre passe-bas de l'amplificateur classe D.}   
    \end{minipage} \hfill
    \begin{minipage}[c]{.46\linewidth}
        \includegraphics[scale=0.6]{eps/Schematique_All.jpg}
        \caption{Filtre passe-haut de l'amplificateur classe D.}    
    \end{minipage}
    \label{fig:classeD}
\end{figure}
...
\textbf{N.B.} : Se référer à la figure 4 de la page 6 pour la notation des deux résistances et des deux condensateurs.
...
\item[ Pour le filtre passe-bas :] Durant le laboratoire, nous avons commencé par dimensionner le filtre passe-bas. La fréquence de coupure doit se trouver aux environ de 20 000 Hz.
	\begin{enumerate}
		...
		\item Poser comme hypothèse que nous souhaitons un gain unitaire. Cela signifie que la valeur de la résistance R13 est égale à la valeur de la résistance R14. En effet : \begin{center} $A_{v}=\frac{R_{2}}{R_{1}} = 1$ \end{center} 
		\item Fixer une valeur standard pour R13 et ré-écrire la formule permettant de trouver la valeur de C18. Nous avons pris une valeur standard de 8200 Ohm pour la résistance R13.
			\begin{center} $R_{1}= R_{2} = 8 200\Omega $ \end{center}
			\begin{center} $C_{2}=\frac{1}{2*\pi*R_{2}*f_{c}}$ \end{center} 
		\item Cherchant une valeur la plus standardisée possible pour le condensateur C18, nous devons ajuster la valeur de la fréquence de coupure dans la formule donnée précédemment. Ceci nous conduit à ajuster la fréquence de coupure à 19 400 Hz.
			\begin{center} $C_{2}=\frac{1}{2*\pi*8 200*19 400}= 1 nF$ \end{center} 
	\end{enumerate}
\end{description}

\begin{description}
\item[ Pour le filtre passe-haut :] La fréquence de coupure doit se trouver aux environ de 20 Hz. Nous connaissons déjà les valeurs standards des deux résistances R13 et R14 ainsi que du condensateur C18. Il nous reste donc à trouver la valeur standard du condensateur C1.
	\begin{enumerate}
		...
		\item Connaissant la valeur de R13, nous trouvons la valeur de C1 en ajustant la valeur de la fréquence de coupure. Nous trouvons finalement une fréquence de coupure de 18 Hz. 
			\begin{center} $C_{1}=\frac{1}{2*\pi*R_{2}*f_{c}}=\frac{1}{2*\pi*8200*18} = 0,001mF$ \end{center} 
	\end{enumerate}
\end{description}

Le tableau ci-dessous reprend les valeurs calculées pour le filtre passe-haut ainsi que pour le filtre passe-bas :
\begin{figure}
    \centering
...
\begin{figure}
    \centering
    \includegraphics[scale = 0.57]{eps/Resultat_filtres.jpg}
...
\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{eps/Bode_Filtres.eps}
    \caption{Réponse fréquentielle pour les filtres passe-bas et passe-haut de l'amplificateur classe D.}
    \label{fig:classeD}
\end{figure}
