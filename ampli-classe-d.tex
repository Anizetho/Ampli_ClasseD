...
\begin{itemize}
\item \noindent\textbf{L'intégrateur pur} : Le montage intégrateur pur, présenté à la figure 7 ci-dessous, est un montage électronique dont le signal de sortie $V_{out}$ correspond à l'intégrale du signal d'entrée $V_{in}$. L'équation suivante traduit la relation d'intégration entre sortie et entrée : 
\begin{gather}
...
\item \noindent\textbf{Le comparateur} : Le comparateur, présenté à la figure 8 ci-dessous, n'est ni plus ni moins qu'un amplificateur opérationnel dont le fonctionnement est le suivant : Si la tension appliquée sur l'entrée non inverseuse $V_{in}$ dépasse la tension appliquée sur l'entrée inverseuse $0V$, la sortie se retrouve à 1. Dans le cas contraire, la sortie se trouve à 0. L'équation suivante traduit cette relation entre l'entrée et la sortie du comparateur : 
\begin{gather}
\begin{cases}
 V_{out} = V^{+} \text{ si } V_{in} > 0\\ 
 V_{out} = 0  \text{ si } V_{in} < 0
\end{cases}
\end{gather}
\begin{figure}[!ht]
	\centering
...
\begin{figure}[htbp]
	...
	\caption{Capture d'écran d'oscilloscope du signal de commande de transistors.
			 Le signal numérique est la modulation Sigma-Delta du signal analogique.
			 On peut constater que le plafond du signal numérique n'est pas stable.}
...
\begin{figure}[htbp]
	...
	\caption{Capture d'écran d'oscilloscope à la sortie du comparateur \texttt{U3A}.
			 On constate que la comparaison change d'amplitude à chaque passage par zéro.}
...
\begin{figure}[htbp]
	...
	\caption{Capture d'écran d'oscilloscope des signaux dédoublés de grilles.
			 Le signal supérieur est le signal du transistor MOS supérieur et
			 réciproquement pour le signal inférieur.
			 On constate que les signaux sont relativement opposés.}
...
\begin{figure}[htbp]
	...
	\caption{Capture d'écran d'oscilloscope de la comparaison entre consigne et modulation.
			 La modulation ne représente que partiellement la consigne, cette dernière ne
			 peut pas contenir toute l'information nécessaire à la reconstruction.}
