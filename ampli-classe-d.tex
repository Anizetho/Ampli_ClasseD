...
Pour se débarasser des fréquences hors de cette bande, nous filtrons le signal.
Il existe 2 familles de filtres selon les composants électroniques utilisés :
\begin{itemize}
    \item\textbf{Les filtre passifs} sont réalisés avec seulement des composants
        passifs : principalement des résistances, des inductances et des condensateurs.
        Leur gain ne peut dépasser 1.
    \item\textbf{Les filtres actifs} contiennent des composants actifs : des
        transistors et des amplificateurs opérationnels.
        Ils peuvent amener de la puissance et donc un gain supérieur à 1.
\end{itemize}
Dans notre circuit, nous utilisons un topologie de Sallen-Key qui consiste en un filtre actif de type Butterworth~\cite{Horowitz:2015aa}.
...
\begin{itemize}
    \item\textbf{Les filtre passe-bas} laisse passer toutes les fréquences depuis
        la fréquence nulle jusqu'à la fréquence de coupure et atténue toutes les
        fréquences supérieures à cette fréquence de coupure.
    \item\textbf{Les filtre passe-haut} atténue toutes les fréquences depuis la
        fréquence nulle jusqu'à la fréquence de coupure et laisse passer toutes les
        fréquences supérieures.
\end{itemize}

Pour rappel, la fréquence de coupure (notée $f_c$) d'un filtre est la fréquence limite de fonctionnement utile de ce filtre.
...
\begin{itemize}
    \item La $f_c$ du filtre passe-bas a \SI{22000}{\hertz}.
    \item La $f_c$ du filtre passe-haut a \SI{20}{\hertz}.
\end{itemize}
...
La figure 3 ci-dessous présente le circuit schématique de la partie pré-amplification et filtre reçu au cours.
Le trajet emprunté par le signal est le suivant : le signal analogique d'entrée arrive sur la pin \verb$ININVA$.
Étant donné la nécessité d'un filtre passe-haut, l'ajout d'un condensateur entre les pins \verb$ININVA$ et \verb$ININVB$ est inévitable.
Une fois qu'il a traversé le condensateur, le signal traverse la résistance \verb$R14$.
Nous retrouvons ensuite un amplificateur opérationnel avec rétro-action (\verb$C18$ et \verb$R13$) dont la sortie, dénommé par \verb$OUTINV$, renvoie un signal filtré (fréquence comprise entre \num{20} et \SI{22000}{\hertz}).
À noter que la résistance \verb$R15$ n'est pas employée.
...
\begin{figure}[htbp]
    ...
    \caption{Schématique de la partie pré-amplification et filtre de l'amplificateur classe D. 
             Le signal d'entrée analogique arrive sur \texttt{ININVA}.
             Ce signal poursuit son chemin tout d'abord à travers le condensateur placé
             entre \texttt{ININVA} et \texttt{ININVB} et ensuite à travers la résistance
             \texttt{R14}.
             On retrouve le signal filtré à la sortie de l'amplificateur avec rétro-action
             (\texttt{C18} et \texttt{R13}) soit \texttt{OUTINV}.
             À noter que la résistance \texttt{R15} n'est pas employée.}
\end{figure}
...
\begin{description}
\item[Pour le filtre passe-bas :] durant le laboratoire, nous avons commencé par
...
\item[Pour le filtre passe-haut :] la fréquence de coupure doit se trouver aux environ
    de 20 Hz.
    ...
    \begin{equation}
        C_1 = \frac{1}{2\pi R_2 f_c} = \frac{1}{2\pi\,8200\times18} = 1uF
    \end{equation}
...
Voulant savoir si les valeurs théoriques calculées au point~\ref{sec:etudeTheorique}
conduisent à des filtres de fidèle qualité, nous les avons testé.
Nous avons donc tracé, dans le diagramme de Bode, la réponse fréquentielle de chaque filtre (\textit{i.e.} passe-bas et passe-haut).
...
Nous avons réalisé un script en Matlab\textregistered{} reprenant toutes les valeurs mesurées et reprises dans les tableaux ci-dessus afin de représenter la réponse fréquentielle de chacun des filtres dans le diagramme de Bode. La figure~\ref{fig:resultatRepFreq} présente le résultat obtenu :
\begin{figure}[htbp]
    \centering
